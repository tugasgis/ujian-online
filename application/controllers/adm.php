...
class Adm extends CI_Controller {
	...
	public function m_siswa() {
		...
		if ($uri3 == "det") {
			...
			if (!empty($det_user)) {
				$this->db->query("INSERT INTO m_admin VALUES (null, 'siswa".$det_user->id."', md5('Siswa".$det_user->id."'), '', '".$det_user->id."')");
				$ret_arr['status'] 	= "ok";
...
$this->cek_aktif();
		...
		$this->load->model('m_register');
        $data1 = $this->m_register->bacadata2();
		$a['p']			= "v_konfrimasi";
	...
	public function act_login() {
		...
		if ($j_data === 1) {
			...
			if ($a_data->level == "siswa") {
				...
				$sess_nama_user = "Pusat At-Tashdiq Indonesia";
			}
			...
			$_log['log']['status']			= "0";
			$_log['log']['keterangan']		= "Maaf, username dan password salah";
